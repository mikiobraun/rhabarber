...
any_t eval(any_t env, any_t expr)
{
  ...
  case _TUPLE_T: 
    // function call
    assert(UNWRAP_PTR(_TUPLE_T, expr));
    return eval_args_and_call_fun(env, UNWRAP_PTR(_TUPLE_T, expr));
  case _STRING_T:
    // string literals (must be copied, since they might be modified)
    return WRAP_PTR(_STRING_T, string_copy(UNWRAP_PTR(_STRING_T, expr)));
  case _BOOL:
  case _INT:
  case _REAL_T:
...
bool pattern_valid(any_t p)
{
  ...
  if (thetyp) return (thelit == 0 || ptype(thelit) == _SYMBOL_T);
...
//     false --> don't know
{
  ...
  // and we return false
  if (thelit1 && ptype(thelit1)!=_SYMBOL_T) 
    return (!thelit2 || ptype(thelit2)==_SYMBOL_T);
  // CASE 2
...
{
  if (ptype(s1)!=_TUPLE_T || ptype(s2)!=_TUPLE_T)
    rha_error("some signatures are broken");
  //print("%s against %s", to_string(s1), to_string(s2));
  tuple_t sig1 = UNWRAP_PTR(_TUPLE_T, s1), sig2 = UNWRAP_PTR(_TUPLE_T, s2);
...
any_t construct_fun_scope(any_t this, any_t fn, tuple_t values, any_t scope)
{
  ...
  assign(local, args_sym, WRAP_PTR(_TUPLE_T, values)); // all passed arguments
...
any_t vcallslot(any_t obj, symbol_t slotname, tuple_t args)
{
  ...
  //debug("callslot(%o, %o, %o)\n", obj, WRAP_SYMBOL(slotname), WRAP_PTR(_TUPLE_T, args));
...
{
  any_t o = lookup(x, s);
  if (!o || ptype(o) != _TUPLE_T)
    rha_error(msg);
  return UNWRAP_PTR(_TUPLE_T, o);
}
