...
any_t create_pattern(any_t theliteral, any_t thetype)
{
  ...
  if (theliteral 
      && ptype(theliteral) != RHA_symbol_t
      && ptype(theliteral) != RHA_bool
      && ptype(theliteral) != RHA_int
      && ptype(theliteral) != RHA_real_t)
...
any_t create_fn_data_entry(any_t env, tuple_t signature, any_t fnbody)
{
  ...
  for (i = 0; i < tlen; i++) {
    ...
    // colon function
    if (ptype(entry) == RHA_tuple_t) {
      tuple_t entry_t = UNWRAP_PTR(RHA_tuple_t, entry);
      if (tuple_len(entry_t) == 3) {
	// (colon_fn int x)
	any_t entry_t0 = tuple_get(entry_t, 0);
	if (ptype(entry_t0) == RHA_symbol_t || UNWRAP_SYMBOL(entry_t0) == colon_fn_sym) {
  ...
  // built fn_data entry
  any_t entry = new();
  assign(entry, signature_sym, WRAP_PTR(RHA_tuple_t, signature));
  if (env)
...
any_t macro_fn(tuple_t argnames, any_t fnbody)
{
  ...
  assign(m, argnames_sym, WRAP_PTR(RHA_tuple_t, argnames));
...
tuple_t map_fn(any_t this, any_t f, tuple_t t)
{
  ...
  for (int i = 0; i < tlen; i++) {
    ...
    if (ptype(entry) == RHA_tuple_t) {
      list_t l = tuple_to_list(UNWRAP_PTR(RHA_tuple_t, entry));
    ...
    tuple_set(sink_t, i, eval(this, WRAP_PTR(RHA_tuple_t, call_t)));
  }
  return sink_t;
}
...
any_t while_fn(any_t this, any_t cond, any_t body)
{
  ...
  begin_frame(LOOP_FRAME)
    while (1) {
      ...
      if (!cond_o || ptype(cond_o)!=RHA_bool)
...
any_t split_resolve_and_eval(any_t root, list_t source, string_t context)
{
  ...
  while ((obj = list_popfirst(l))) {
    ...
    try {
      expr = resolve(root, UNWRAP_PTR(RHA_list_t, obj));
...
any_t literal(any_t env, list_t parsetree)
{
  ...
  return WRAP_PTR(RHA_list_t, sink);  // a list!
}
