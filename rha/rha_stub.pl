...
if ($#ARGV == -1) {
    # print usage
    die "USAGE: rha_stub.pl test_tr.h\n";
}

# usage: rha_stub.pl test_tr.h
$hfilename = shift @ARGV;
$debug = shift @ARGV;
...
$ops = "plusplus|minusminus|plus|minus|times|divide|less|greater|equal|not|and|or|lessequal|greaterequal";

# mapping from C types to rhabarber types
%c2rha = ("bool"     => "bool", 
	  "int"      => "int", 
	  "double"   => "real",
	  "string_t" => "string");
%rha2c = reverse %c2rha;
$ctypes = join ', ', keys %c2rha;

# known keywords
$keywords = "constructor|method|operator|constant";

# some stuff for beautifications
@ordinal = ("th", "st", "nd", "rd", "th");

# check format of hfilename
if ($hfilename !~ /($id)\_tr.h/) {
    die "Input file name must end on _tr.h, e.g. test_tr.h";
}
$class = $1;
